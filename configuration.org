...
(setq mpereira/light-theme 'material-light)
(setq mpereira/dark-theme 'material)
...
(with-eval-after-load "lispy"
  ...
  (defun mpereira/maybe-move-to-after-string ()
...
in any structured movement package is mind-boggling to me."
    (interactive "p")
    ;; Prevent moving past the end of a list.
    (unless (looking-at (concat "." lispy-right))
      (progn
        (if-let (bounds (lispyville--in-string-p))
            ;; Move to point right after string end.
            (progn
              (goto-char (cdr bounds))
              (forward-char 1))
          (when-let (bounds (lispy--bounds-dwim))
            ;; Move to point right after symbol or list end.
            (goto-char (cdr bounds))
            (forward-char 1)))
        (forward-sexp 1)
        (backward-char 1)))))
...
(use-package ivy-posframe
  ...
  ;; TODO: Disable for now. Width-changing at-point frames are annoying. Has
  ;; some bugs with fullscreen.
  (ivy-posframe-mode -1))
#+END_SRC
