...
(setq mpereira/dark-theme 'doom-Iosvkem)
#+END_SRC
...
(use-package evil
  ...
  ;; `evil-search' as the `evil-search-module', but I still want to navigate
  ;; history with C-j and C-k.
  ;; (general-define-key
  ;;  :keymaps '(isearch-mode-map evil-ex-search-keymap)
  ;;  "C-k" #'isearch-ring-retreat
  ;;  "C-j" #'isearch-ring-advance)

  ;; FIXME: "n" and "N" still cause the highlights to persist. Maybe it's
  ;; related to the difference between `evil-ex-search-forward' and
  ;; `evil-search-forward'.
...
(use-package evil-collection
  ...
  :after evil
  :config
  (evil-collection-init '(package-menu dired disk-usage magit-todos))
