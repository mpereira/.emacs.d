...
;; FIXME: Using `toggle-frame-maximized' instead of `toggle-frame-fullscreen'
;; because of
...
|        6 | asc   | ITEM     |"
  ...
  (let* ((todo-max (apply #'max (mapcar #'length org-todo-keywords)))
         ...
         (alltags (or (org-entry-get (point) "ALLTAGS") alltags-default))
...
(defvar mpereira/org-agenda-previous-window-configuration nil
  "A window configuration to return to when closing the org agenda.")

(defvar mpereira/org-agenda-previous-point nil
  "A point to return to when closing the org agenda.")
...
(defun mpereira/open-or-build-org-agenda ()
  "Display org agenda if it was already built. Build and display it otherwise."
  (interactive)
  (let* ((org-agenda-buffer-name "*Org Agenda*")
         (org-agenda-buffer (get-buffer org-agenda-buffer-name)))
    (setq mpereira/org-agenda-previous-window-configuration
          (current-window-configuration))
    (setq mpereira/org-agenda-previous-point (point))
...
(defun mpereira/close-org-agenda ()
  "Close the org agenda and restore the previous window configuration and point
position."
  (interactive)
  (set-window-configuration mpereira/org-agenda-previous-window-configuration)
  (setq mpereira/org-agenda-previous-window-configuration nil)
  (goto-char mpereira/org-agenda-previous-point)
  (setq mpereira/org-agenda-previous-point nil))
#+end_src
