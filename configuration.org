...
:END:
  ...
  -  [[#org][Org]]
    -  [[#org-mode][org-mode]]
    -  [[#sort-org-entries-by-multiple-fields][Sort org entries by multiple fields]]
    -  [[#org-clock][Org clock]]
    -  [[#org-gcal][org-gcal]]
...
** Sort org entries by multiple fields
...
5. asc  ITEM"
  ...
  (let* ((todo-max (apply #'max (mapcar #'length org-todo-keywords)))
         ...
         (created-int-min 10000000000000) ; YYYY=1000 mm=00 dd=00 HH=00 MM=00 SS=00
         (created-int-max 30000000000000) ; YYYY=3000 mm=00 dd=00 HH=00 MM=00 SS=00
         ...
         (sort-key (format "%03d %s %03d %.10f %s"
                           todo-int
                           alltags
                           priority-int
                           (/ (float created-int-max) created-int)
                           item)))
    (message sort-key)
    sort-key))
...
My agenda is a bit heavy to build so I don't kill its buffer (I use ~bury-buffer~
...
I'm also planning to add automatic and periodic background refreshing of the
agenda buffer with ~run-with-idle-timer~ soon.
